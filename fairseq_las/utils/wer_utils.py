...
def trim_white_space(sentence):
    return re.sub(" +", " ", re.sub(" *$", "", re.sub("^ *", "", sentence)))


def str2toks(sentence):
    pieces = trim_white_space(sentence).split(" ")
    tokens = list()

    for piece in pieces:
        tokens.append(Token(piece, 0.0, 0.0))
    return tokens
