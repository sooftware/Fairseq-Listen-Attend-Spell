...
class Seq2SeqCollater(object):
    ...
    def collate(self, samples):
        ...
        for sample in samples:
            ...
            if sample["data"][self.feature_index] is None:
                continue

            source = sample["data"][self.feature_index]

            if isinstance(source, (np.ndarray, np.generic)):
                source = torch.from_numpy(source)

            target = sample["data"][self.label_index]
            ...
            parsed_sample = {"id": sample["id"], "source": source, "target": target}
